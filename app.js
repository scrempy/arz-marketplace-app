// Отлов критических ошибок (чтобы понимать, почему не работают кнопки)
window.onerror = function(msg, url, lineNo, columnNo, error) {
    alert("Критическая ошибка JS: " + msg + " (Строка: " + lineNo + ")");
    return false;
};
...
try {
    ...
    window.openSrv = function() { document.getElementById('srv-modal').style.display='block'; }
    window.selectSrv = function(s) { document.getElementById('current-srv-name').innerText=s; document.getElementById('srv-modal').style.display='none'; loadFeed(s); }
    ...
    window.submitCreate = async function() {
        ...
        if(globalTimers.manualSec > 0 && userId !== OWNER_ID) return tg.showAlert(`КД! Подождите еще ${formatTime(globalTimers.manualSec)}`);
    ...
    document.getElementById('e-save-btn').onclick = async () => {
        ...
        try {
            ...
            if(res.ok) { 
                closeEdit(); loadFeed('Все'); tg.showAlert("Сохранено!"); 
            }
        } catch(e) { tg.showAlert("Ошибка связи с сервером"); }
    ...
    window.openAutoPR = async function() {
        showPage('auto');
        if(role === 0 && userId !== OWNER_ID) { document.getElementById('auto-lock').style.display='block'; return; }
    ...
    alert("Критический сбой: " + globalError.message);
}
