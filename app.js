...
window.openAutoPR = () => {
    ...
    if (currentRole === 0 && userId !== OWNER_ID) {
        document.getElementById('auto-lock').style.display = 'block';
        document.getElementById('auto-content').style.display = 'none';
    } else {
        document.getElementById('auto-lock').style.display = 'none';
        ...
        ads.forEach(ad => {
            ...
            <div class="ad-card ${cardClass}" id="ad-${ad.id}">
                ...
                ${isMod ? `<div class="action-row">
                    <button class="btn-edit" onclick='window.openEdit(${JSON.stringify(ad).replace(/'/g, "&apos;")})'>📝 Изменить</button>
